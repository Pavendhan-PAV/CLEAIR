...
if(isset($_POST["fID"]) && isset($_POST["password"]) && !isset($_SESSION['fID2'] ) )
	{
		
		
		$fID = $_POST["fID"];
		$sql = "SELECT fpassword FROM farmers WHERE fID='$fID'";
		$query = $pdo->query($sql);
	
		if($query)
		{
			$row = $query->fetch(PDO::FETCH_ASSOC);
			if($row['fpassword']==$_POST["password"])
			{
...
?>
		...
		<input type="text" placeholder="ID"  style="color: #1b1b1b; text-transform: capitalize;" required="" name="fID">
		<input type="text" placeholder="USERNAME"  style="color: #1b1b1b; text-transform: capitalize;" required="" name="fusername">
