<?php
	...
	if(isset($_POST["name"]))
	{
		$employee=mysqli_connect("127.0.0.1","root","root","cleair2");
		$id=$_POST["name"];	
		$user=$_POST["username"];
		$sql="SELECT * from industry where iID = '$id'";
			...
			if(!$query->fetch(PDO::FETCH_ASSOC))
			{
				...
				$insert="INSERT INTO industry(iID,iEmailId,icontact,ilocation,ipin,icity,iindustry_name,ipassword) 
									values('$name','$email','$contact','$landmark','$pincode','$city','$username','$password') ";
				
				$pdo->query($insert);

				header("Location: i_log.php");
				return;
			}
			else
			{
				$_SESSION["error"] = "error";
				header("Location: i_signup.php");
				return;
			}
...
<div class="header" >
	...
	<!---->
	<div class="reg" style="margin-bottom: 250px;">
		<div class="container">
	...
	<div class="ser-top ser-top-c" style="margin-bottom: 40px; background: transparent;">
					...
					<h2 style="font-weight: normal;color: white;">Industrialist Signup</h2>
		...
		<input type="text" placeholder="Location" style="color: #1b1b1b;" name="landmark"><br><br>
		<input type="tel" placeholder="PINCODE" required="" style="color: #1b1b1b;" pattern="[1-9]{1}[0-9]{2}\s{0,1}[0-9]{3}" name="PINCODE"><br><br>
